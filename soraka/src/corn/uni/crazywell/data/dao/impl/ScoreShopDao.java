...
@Named
@Stateless
public class ScoreShopDao extends AbstractGenericDAO<ShopScoreEntity> implements ScoreDao {
    public ScoreShopDao(){
    ...
    public boolean isVoted(Bubble bubble) throws DAOException {
        try {
            String rq = "select score.value from ShopScoreEntity score where score.shopId = " + bubble.getBody().get(2).toString() + " and score.uuid = " + bubble.getBody().get(1).toString();
