...
@Named("scoreShowDao")
@Stateless
public class ScoreShowDAO extends AbstractGenericDAO<ShowEntity> implements GenericDAO<ShowEntity>, ScoreDAO {
    @Override
    public boolean isVoted(Bubble bubble) throws DAOException {
        try{
            final Query query = entityManager.createQuery("select score.value from ShowScoreEntity score where score.showId = :idShow and score.uuid = :uuid");
            query.setParameter("idShow", bubble.getBody().get(2).toString());
